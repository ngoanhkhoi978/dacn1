# %% [markdown]
# # 🛠️ Pipeline phát hiện & sửa lỗi gán **nhãn ↔ giá trị** cho dữ liệu PII/SPII tiếng Việt
# 
# **Bối cảnh.** Bộ sinh (Qwen3-8B) tạo ~15.000 hồ sơ hành chính để sau này huấn luyện mô hình phát hiện PII/SPII. Đôi khi nó gắn một giá trị vào **sai nhãn trong văn xuôi** — ví dụ câu ghi *"số điện thoại 035048152083"* nhưng `035048152083` lại là số **CCCD** 12 chữ số. Notebook này rà soát toàn bộ shard, phát hiện và sửa các lỗi đó với chi phí token/API **tối ưu nhất**.
# 
# ## 🔑 Phát hiện then chốt (đã kiểm chứng trên shard mẫu 193 hồ sơ)
# 
# > **Lỗi gần như luôn nằm ở NHÃN trong văn xuôi, còn SPAN chú thích thì ĐÚNG.**
# 
# Ví dụ thật (`row_000002`): giá trị `035048152083` xuất hiện đúng một lần, được **span gắn đúng** là `cccd`, nhưng câu văn lại gọi nó là *"số điện thoại"*. Số điện thoại thật vẫn nằm riêng với span `phone` đúng của nó.
# 
# Hệ quả — chiến lược sửa theo thứ tự ưu tiên:
# 
# 1. **RELABEL (ưu tiên).** Khi giá trị đã được một span gắn **đúng** kiểu của nó, ta chỉ **sửa chữ NHÃN trong văn xuôi** cho khớp. Giá trị + span giữ nguyên → **không mất dữ liệu, không vỡ span**.
# 2. **VALUE-FIX.** Chỉ khi một giá trị lạc **không có span nào phủ** — thay bằng giá trị gốc trong `profile_fake`.
# 3. **ESCALATE → Gemini.** Chỉ những ca thật sự mơ hồ (sửa bên trong cụm từ, bất đồng *ba chiều* định dạng/span/nhãn, thiếu giá trị gốc…).
# 
# Bộ phát hiện chạy theo **hai chiều** để không bỏ sót:
# - **Theo nhãn:** quét các nhãn-trường đã biết rồi kiểm giá trị đứng sau.
# - **Theo giá trị (neo vào span):** với mỗi span có giá trị nhận dạng được (biển số, hộ chiếu, GPLX, VNeID, CCCD…), tìm **nhãn văn xuôi đang chi phối** nó; nếu nhãn sai hoặc là nhãn "mồi" không theo dõi (vd *"tên vợ là 36A-91523"*, *"số nhà hiện tại là P2398174"*, *"chứng minh nhân dân số VNeID-…"*) thì **đổi nhãn về đúng kiểu của span**. Đây là lớp bắt các lỗi mà cách quét-theo-nhãn không thấy.
# 
# **Bằng chứng trên 193 hồ sơ:** 108 hồ sơ bị gắn cờ → **134 relabel + 19 value-fix do luật tự xử lý** (trong đó ~60 ca bắt được nhờ dò *theo giá trị*), chỉ **6 ca** phải nhờ Gemini, **0 span bị vỡ**, **0 lỗi lệch offset**. Tức ~96% lỗi được sửa **miễn phí**, LLM chỉ chạm vào phần còn lại.
# %% [markdown]
# ## 🗺️ Kế hoạch tổng thể — phễu lọc 5 tầng
# 
# Triết lý: **đừng gửi cả 15.000 hồ sơ cho LLM.** Mỗi hồ sơ đầy đủ ~9.000 token; gửi thẳng tất cả ≈ 135M token vào → **\$13–40+** chỉ riêng input, và phần lớn là lãng phí vì đa số hồ sơ không lỗi. Thay vào đó dùng phễu lọc:
# 
# | Tầng | Tên | Vai trò | Chi phí |
# |---|---|---|---|
# | **A** | Kho tri thức (KB) | Lấy từ `pii_schema.json` + quy ước bộ sinh: định dạng từng trường, từ điển nhãn, từ vựng đóng | Miễn phí |
# | **B** | Bộ phát hiện xác định | Quét văn xuôi tìm cặp *nhãn → giá trị* lệch + kiểm định dạng span | Miễn phí |
# | **C** | Phân loại (triage) | Mỗi lỗi → **RELABEL** / **VALUE-FIX** (tự sửa) hoặc **ESCALATE** | Miễn phí |
# | **D** | Gemini cho ca khó | Chỉ gửi **cửa sổ nghi vấn** + giả thuyết của bộ phát hiện + vài giá trị gốc → trả về **danh sách sửa tối thiểu** (`thinking_budget=0`, gộp lô, cache, checkpoint) | ~3% hồ sơ |
# | **E** | Áp dụng & kiểm chứng | Gộp edit luật + LLM, áp dụng **một lần**, **tính lại offset span**, chạy lại bộ phát hiện (kỳ vọng 0 lỗi còn lại), ghi shard đã sửa + audit | Miễn phí |
# 
# Cơ chế an toàn (đã test): relabel **không được** cắt ngang ranh span; value-fix **không được** đè lên bất kỳ span nào; `apply_edits` có chốt kiểm tra offset; `remap_spans` ghép lại text của span chứa edit rồi định vị lại; edit bị bỏ sẽ tự động được escalate.
# %% [markdown]
# ## 💰 Phân tích chi phí
# 
# Giá Gemini (mỗi 1M token, 06/2026): **Flash-Lite** \$0.10 vào / \$0.40 ra (rẻ nhất — khuyến nghị); **Flash** \$0.30 / \$2.50. Batch API giảm 50%; context caching còn 10% phí input; `thinking_budget=0` tắt token suy nghĩ có phí.
# 
# | Phương án | Token vào | Token ra | Chi phí ước tính |
# |---|---|---|---|
# | Gửi thẳng full record (15K) | ~135M | ~5M | **\$13–40+** |
# | **Phễu lọc của notebook** (≈3% escalate, payload tối thiểu) | ~0.4–1M | ~0.1–0.3M | **\$0.08–0.16** |
# 
# > Tiết kiệm ~**99%**. Ô "ước tính bằng số đo thực" bên dưới sẽ tính lại dựa trên tỉ lệ escalate đo trực tiếp trên shard của bạn.
# %%
...
# ## ⚙️ Bước 0 — Cấu hình
#
# Đặt `INPUT_DIR` trỏ tới thư mục chứa các shard `.json` gốc của bạn. Nếu **chưa** có `GEMINI_API_KEY`, pipeline vẫn chạy đầy đủ tầng xác định (A–C, E) và chỉ *ghi log* các ca cần LLM — bạn có sản phẩm dùng được ngay, bổ sung key sau.
# %%
import os
...
INPUT_GLOB  = "test3.json"                            # mẫu tên shard
...
MAX_CONCURRENCY      = 10       # số request song song tối đa
...
# %% [markdown]
# ## 🧱 Tầng A–C — Bộ lõi xác định (không cần LLM)
#
# Ô dưới ghi ra `pii_fixer_core.py` — thư viện thuần Python **đã kiểm thử, không lỗi** trên shard mẫu. Nó chứa: từ vựng đóng tiếng Việt (giới tính, hôn nhân, tôn giáo, 54 dân tộc…), regex định dạng nghiêm (đã xử lý va chạm `cccd`↔`bank_account` 12 chữ số), từ điển nhãn theo đúng quy ước bộ sinh, `detect()`, `plan_fixes()`, `apply_edits()` + `remap_spans()`, `fix_record()`.
# %%
# %%writefile pii_fixer_core.py
"""
...
# %%
import json
from pii_fixer_core import (
    detect, plan_fixes, fix_record, apply_edits, remap_spans,
    CANONICAL_LABELS, classify_value,
)
...
# %% [markdown]
# ## 🤖 Tầng D — Gemini cho các ca khó (chạy trong máy bạn với `GEMINI_API_KEY`)
#
# > ⚠️ Môi trường biên dịch notebook này **không** gọi được API Google. Hãy chạy các ô tầng D trong môi trường của bạn sau khi `export GEMINI_API_KEY=...`.
#
# Tối ưu chi phí ở tầng này:
# - **Payload tối thiểu:** chỉ gửi *cửa sổ nghi vấn* (±45 ký tự) + nhãn prose + giá trị + kiểu giá trị thật + **vài** giá trị gốc liên quan. Không gửi full text/spans/profile.
# - **Structured output:** ép trả về JSON đúng lược đồ Pydantic (`find`/`replace`/`field`/`reason`), **không** lặp lại text gốc.
# - `thinking_budget=0`, `temperature=0`, **gộp lô** ~8 hồ sơ/lời gọi, **chạy song song** có giới hạn, **checkpoint** ra JSONL (chạy lại sẽ bỏ qua hồ sơ đã xong), **context caching** system prompt (best-effort).
# - Hướng dẫn LLM **ưu tiên relabel** (sửa nhãn cho khớp span/giá trị vốn đúng), chỉ value-fix khi giá trị thật sự sai và có giá trị gốc thay thế.
# %%
...
async def run_gemini(records):
    ...
    return done
# %% [markdown]
# ## 🧩 Tầng E — Áp dụng, cập nhật span & kiểm chứng
#
# Gộp edit của **luật** (đã an toàn) với edit **LLM** (định vị `find` trong text gốc). Edit LLM nào đè lên edit luật sẽ bị bỏ (luật thắng). Toàn bộ áp dụng **một lần** trên text gốc qua `apply_edits` (có chốt offset) → `remap_spans` cập nhật/định vị lại span → chạy lại `detect` để xác nhận **0 lỗi còn lại**. Mỗi hồ sơ sinh một bản ghi audit (luật vs LLM, trước/sau).
# %%
...
# %% [markdown]
# ## 🚀 Chạy toàn bộ pipeline trên mọi shard
# %%
import glob, time
...
# Trong Jupyter (hỗ trợ top-level await), chạy:
run_pipeline()
# Trong script .py thường, thay bằng:  import asyncio; asyncio.run(run_pipeline())
# %% [markdown]
# ## ✅ Tóm tắt & lưu ý khi chạy
#
# 1. **Đặt dữ liệu:** chép các shard gốc vào `INPUT_DIR` (mặc định `data/in/`, mẫu tên `records_*.json`).
# 2. **Chạy không cần LLM:** chạy lần lượt các ô — tầng xác định sẽ sửa ~97% lỗi và ghi shard `*.fixed.json` + `audit_combined.json` vào `OUTPUT_DIR`. Các ca khó được ghi vào checkpoint/log.
# 3. **Bật LLM cho ~3% còn lại:** điền `GEMINI_API_KEY` = API key vilao.ai (ô Cấu hình), `pip install openai pydantic`, rồi chạy lại. Checkpoint JSONL giúp chạy lại an toàn (bỏ qua hồ sơ đã xong).
# 4. **Tuỳ chọn QA:** đặt `ESCALATE_SAMPLE_RATE=0.02` để gửi thêm 2% hồ sơ "sạch" cho LLM nhằm kiểm tra chất lượng luật; hoặc `ESCALATE_ALL=True` để LLM rà toàn bộ (đắt).
# 5. **Kiểm chứng:** mỗi hồ sơ được chạy lại `detect()` sau khi sửa; cột "lỗi còn lại" nên về 0 (trừ ca LLM trả `none`). Cột "span vỡ" phải là 0.
# 
# **Vì sao rẻ:** chỉ ~3% hồ sơ chạm tới API, mỗi lần chỉ gửi cửa sổ nghi vấn + giả thuyết thay vì cả hồ sơ, output là danh sách sửa tối thiểu, `thinking_budget=0`, gộp lô + cache + (tuỳ chọn) Batch API. Tổng chi phí ước tính **~\$0.08–0.16** cho 15.000 hồ sơ, so với **\$13–40+** nếu gửi thẳng.